...
h1: Active Directory as an SSO provider for SSH Authentication
---
...
- Devs are only allowed to log in to nodes labelled with the `access: relaxed` label.
- Developers can log in as the `ubuntu` user.
- Developers they will not be able to see or replay past sessions or
