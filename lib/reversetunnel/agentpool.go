...
type AgentPoolConfig struct {
	...
	// ConnectedProxies signals which proxy an agent is connected to. An empty
	// list of proxy ids will be used when proxy peering is not enabled by the
	// cluster.
	ConnectedProxies *ConnectedProxies
	// IgnoreProxyPeering ignores the cluster proxy peering setting and connects to all proxies.
	IgnoreProxyPeering bool
}
...
// ConnectedProxies returns the ConnectedProxies for this agent pool.
func (p *AgentPool) ConnectedProxies() *ConnectedProxies {
	return p.AgentPoolConfig.ConnectedProxies
}

func (p *AgentPool) isProxyPeeringEnabled() bool {
	return p.proxyPeeringEnabled && !p.IgnoreProxyPeering
}

func (p *AgentPool) updateConnectedProxies() {
	if !p.isProxyPeeringEnabled() {
...
// start connects agents until the agent pool context is done.
func (p *AgentPool) start() error {
	for {
		if p.ctx.Err() != nil {
...
// handle connects a new agent and processes any agent events blocking until a
...
// isAgentRequired returns true if a new agent is required. This will always
// return true when proxy peering is not enabled.
func (p *AgentPool) isAgentRequired() bool {
	...
	p.log.Debugf("Proxy peering enabled: %v", p.isProxyPeeringEnabled())
	if !p.isProxyPeeringEnabled() {
		return true
	}
...
func (p *AgentPool) updateSettings() error {
	...
	p.proxyPeeringEnabled = config.GetProxyPeering() == types.ProxyPeering_Enabled
	return nil
}
...
func (p *AgentPool) newAgent(ctx context.Context, tracker *track.Tracker, lease track.Lease) (*Agent, error) {
	...
	dialer := &agentDialer{
		client:              p.Client,
		fips:                p.FIPS,
		proxyPeeringSupport: !p.IgnoreProxyPeering,
